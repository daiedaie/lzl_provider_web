                ...
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'">
                            <!-- <i-button type="primary" @click="RowClick(item,$event,index)" key="编辑">编辑</i-button> -->
                            <Poptip
                                confirm
                                title="您确认删除这条内容吗？"
                                width=190
                                white-space: normal
                                placement=top-start
                                @on-ok="RowDelete(item,$event,index)"
                                @on-cancel="cancel">
                                <i-button type="error" key="删除">删除</i-button>
                            </Poptip>
                            <i-button type="primary" key="编辑" @click="editRow(item,$event,index)">编辑</i-button>
                        </div>
                        <div v-if="column.type === 'pic'">
                            <img class='small-pic' :src="imgrul+item.pic"></img>
                        </div>
...
export default {
    ...
    methods: {
      ...
      iconRow() {
        ...
        // },
        // 删除
        RowDelete(data, event, index) {
            let result = this.makeData(data)
            this.$emit('on-row-delete', result, event, index)
        },
        // 编辑
        editRow(data, event, index) {
            //该result就是该行的数据
        	let result = this.makeData(data)
            this.$emit('on-row-edit', result, event, index)
        },
