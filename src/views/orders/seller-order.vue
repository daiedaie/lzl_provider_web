            ...
            <Col>
                <Card>
                    ...
                    <Row>
                        ...
                        <Input v-model="searchSellerMobile" placeholder="请输入商家电话..." class="search-table" />
                        <Select v-model="searchOrderState" placeholder="请选择订单状态..." class="search-table">
                            <Option v-for="item in orderStateList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                        <DatePicker type="daterange" :options="dataSelect" @on-change="getDate" placement="bottom-end" placeholder="请选择时间..." class="search-table"></DatePicker>
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      //response数据处理
      orderAPI.get.r(params).then(res => {
        if(res.data){
            this.data = res.data
        }else{
            this.data = [];
